...
export default class ContentInterestings extends React.Component {
  ...
  render() {
    ...
    var interestings = this.state.interestings
    var bodyTopHeight = 150
    var bodyBottomHeight = (this.props.windowHeight - bodyTopHeight - 80)
    return (
                ...
                return(
                    <div key={i} className="InfoTilesWrapper" data-id={i}>
                        <div className="EditCloseIcon">
                            ...
                            <span className="IconsSpan" onClick={(event)=>this.helpDetails(event,i)}>{val.interestingDetail ? <img className="NotificationIcon" src="images/comment-icon.png" alt="details" />: ""}<img className="Icons" src="images/details.svg" alt="details" /><span className="ToolTipText">details</span></span>
                            { this.state.intId == i && (this.state.action == "edit" || this.state.action == "details") ?<span className="IconsSpan" onClick={(event)=>this.returnToHomeTile(event,i)}><img className="Icons" src="images/previous.svg" alt="return"/><span className="ToolTipText">return</span></span>:""}
                        </div>
                            {   
