...
export default class Router extends Component {
    constructor(props){
        super(props)
        this.state = {route : "loader",standUpData:"",windowHeight:"",windowWidth:""}
    ...
    refineEventList(events){
        ...
        events = events.filter((val)=>{
            ...
            var check,monthSame=0,yearSame=0
            ...
            if(check == 0)
                temp = "Today"
            if(check == -1){
                if(yearSame && monthSame && (today.getDate()+1) == eventDate.getDate())
                    temp = "Tomorrow"
                else
                    temp = "can'tDetermine"
            }
    ...
    render(){
        switch(this.state.route){
            case "standUp" : 
                return (<StandUpHome windowHeight={this.state.windowHeight} standUpData={this.state.standUpData} route={this.state.route} changeRoute={this.changeRoute} refineEventList={this.refineEventList} loadLatestData={this.loadLatestData} loadThenUpdate={this.loadThenUpdate}/>);
